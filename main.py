...
import math
import numpy as np
tutor_initial_csv = pd.read_csv('/Users/isabellechen/Desktop/Mock_Session_Time_Submission_Fo2022-01-12_09_59_44.csv')
student_initial_csv = pd.read_csv('/Users/isabellechen/Desktop/Tutor_Time_Submission_Form2022-01-12_10_01_58.csv')
#test['Mon'].split('\n')#.splitlines()
...
def initial_table(test):
        ...
        for d in range(0, len(days)):
            for nn in range(1, 6):
                if test[f"{days[d]} Time {nn}"][i] != 'PM\nPM' and pd.isna(test[f"{days[d]} Time {nn}"][i]) == False:
                    if test[f"{days[d]} Time {nn}"][i][22:23] == ':' and test[f"{days[d]} Time {nn}"][i][21:22]!='0':  # test['Monday Time 2'][0] '08:00 PM - 09:00 PM (1:00)'
                        if d == 0:
    ...
    return fin_table, name_list, email_list,language_list,interest_list, submission_date, error

def vectorize_table(fin_table, name_list, email_list, language_list,interest_list, submission_date):
    """
                ...
                for m in range(0, len(tutor[days[k]][l])):
                    for n in range(0, len(student[days[k]][j])):
                        if student[days[k]][j]!=[0,0] and tutor[days[k]][l]!=[0,0]:
                            ...
                            if (tutor_start == student_end) or (tutor_end == student_start) or (student_start < tutor_start and student_end < tutor_start) or (student_start > tutor_start and student_start > tutor_end) or ((tutor_start < student_start) and (tutor_end < student_start) and (tutor_end < student_end) and ((student_start - tutor_end) < 1)) or ((student_start < tutor_start) and (student_end < tutor_end) and (tutor_start < student_end) and ((tutor_start - student_end) < 1)):
                                rating += 0
                            else:
                                rating += 1
...
def match(df):
    ...
    ELL_d = {'Student': paired_student, 'Tutor': paired_tutor, 'Language': paired_language,
             'Student Email': paired_student_email, 'Tutor Email': paired_tutor_email,
             'Monday': Monday_time, 'Tuesday': Tuesday_time, 'Wednesday': Wednesday_time, 'Thursday': Thursday_time,
    ...
    removed_df['Order'] = removed_df['Ranking Total'].rank(method='dense',
                                                           ascending=True)  # ordering the column so lower ranking total goes first
    ...
    Paired_d = {'Student': paired_student, 'Tutor': paired_tutor, 'Language': paired_language, 'Student Email': paired_student_email, 'Tutor Email': paired_tutor_email,
                'Monday': Monday_time, 'Tuesday': Tuesday_time, 'Wednesday': Wednesday_time, 'Thursday': Thursday_time,
                'Friday': Friday_time, 'Saturday': Saturday_time, 'Sunday': Sunday_time}
        ...
        for ol in range(0, len(unique_student_list)):
            if len(one_language_students) != 0:
                ...
                if unique_student_list[ol] in new_unique_student_list:
                    ...
                    paired_student_email.append(to_be_paired['Student Email'][0])
                    # Updating the table
                    one_language_students = new_one_language_students
                else:
                    unpaired_student.append(unique_student_list[ol])
    ...
    for k in range(0, len(distinct_list_of_student)):
        if len(removed_df) != 0:
            ...
            if distinct_list_of_student[k] in new_distinct_list_of_student:
                # Selecting one student
                to_be_paired = removed_df[removed_df["Student Name"].isin([distinct_list_of_student[k]])]
                to_be_paired['column_c'] = np.where(to_be_paired['Student Interest'] == to_be_paired['Tutor Interest'], to_be_paired['Rating'], to_be_paired['Rating'] + 5)
                to_be_paired['Order_Rating'] = to_be_paired['Rating'].rank(method='dense', ascending=False)
                to_be_paired = to_be_paired.sort_values(by=['Order_Rating'], ascending=True)
                to_be_paired = to_be_paired.reset_index(drop=True)
...
# /************************************************ Execution Starts ************************************************
print('---- For Tutors -----')
tutor,tutor_name_list, tutor_email_list,tutor_language_list,tutor_interest_list, tutor_submission_date, tutor_error = initial_table(tutor_initial_csv)
print('---- For Students -----')
student,student_name_list, student_email_list,student_language_list,student_interest_list, student_submission_date, student_error= initial_table(student_initial_csv)

tutor_comparison_table = vectorize_table(tutor,tutor_name_list, tutor_email_list,tutor_language_list,tutor_interest_list, tutor_submission_date)
student_comparison_table = vectorize_table(student,student_name_list, student_email_list,student_language_list,student_interest_list, student_submission_date)
        ...
        for i in range(0, len(Paired)):
            ...
            if len(Paired[day][i]) == 0:
                ...
                for ii in range(len(Paired[day][i])):
                    ...
                    start_minute = int((Paired[day][i][ii][0] - start_hour) * 60)
...
Paired_Interest2 = time_convert_format(Paired)
Paired_Interest2 = clean_up(Paired_Interest2)

Paired2.to_csv('/Users/isabellechen/Desktop/Paired.csv')
Unpaired.to_csv('/Users/isabellechen/Desktop/Unpaired.csv')
Paired_Interest2.to_csv('/Users/isabellechen/Desktop/Paired_Interest.csv')
Unpaired_Interest.to_csv('/Users/isabellechen/Desktop/Unpaired_Interest.csv')
df.to_csv('/Users/isabellechen/Desktop/df.csv')
Other_option_df.to_csv('/Users/isabellechen/Desktop/Other_option_df.csv')
